...
export class AvailabilityService {
  ...
  private ensureUTCDate(date: Date): Date {
    ...
    this.logger.debug(`Date conversion: ${date.toString()} → ${utcDate.toISOString()}`);
    return utcDate;
  }

  async addOrUpdateAvailability(teacherId: string, dto: UpdateAvailabilitySlotsDto): Promise<{ message: string }> {
    ...
    const availabilityEntities = dto.availabilities.map(slot => {
      ...
      if (!slot.startTime.includes('Z') && !slot.startTime.includes('+') && !slot.startTime.includes('-')) {
      ...
      const nowWithBuffer = new Date(nowUtc.getTime() + bufferMinutes * 60 * 1000);
